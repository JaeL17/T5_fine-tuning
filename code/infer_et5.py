...
class DataTrainingArguments:
    ...
    test_fpath: Optional[str] = field(default=None, metadata={"help": "Target language id for translation."})
    output_fpath: Optional[str] = field(default=None, metadata={"help": "Target language id for translation."})
    trained_model: Optional[str] = field(default=None, metadata={"help": "Target language id for translation."})
...
# or by passing the --help flag to this script.
# We now keep distinct sets of args, for a cleaner separation of concerns.
def inference(querys):
    print('input: ',querys)
    print(tokenizer(querys.strip())['input_ids'])
    print(tokenizer.convert_tokens_to_ids(tokenizer.tokenize(querys.strip())))

    res_ids = model.generate(
        input_ids = torch.tensor([tokenizer(querys.strip())['input_ids']]),
        attention_mask = torch.tensor([tokenizer(querys.strip())['attention_mask']]),
        num_beams = 5,
        max_length = 200,
        # min_length = 10,
        early_stopping = True,
    ...
    print(answer)
    response = ''.join(answer).replace('<pad>', '').replace('</s>', '').strip()
    return response
